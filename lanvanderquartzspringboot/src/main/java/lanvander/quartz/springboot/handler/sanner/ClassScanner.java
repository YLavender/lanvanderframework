package lanvander.quartz.springboot.handler.sanner;
...
public class ClassScanner implements ResourceLoaderAware {
  ...
  public static Set<Class<?>> scan(
    ...
    ClassScanner classLoader = new ClassScanner();
    if (CollectionUtils.isNotEmpty(annotations))
      Arrays.stream(annotations)
          .forEach(
              annotation -> {
                classLoader.addIncludeFilter(new AnnotationTypeFilter(annotation));
              });
    ...
    Arrays.asList(basePackages)
        .forEach(
            basePackage -> {
              classSet.addAll(classLoader.scan(basePackage));
            });
